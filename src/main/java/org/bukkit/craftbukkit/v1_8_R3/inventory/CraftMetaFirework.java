...
class CraftMetaFirework extends CraftMetaItem implements FireworkMeta {
    ...
    private int power;
    private static int[] $SWITCH_TABLE$org$bukkit$FireworkEffect$Type;
    ...
    CraftMetaFirework(NBTTagCompound tag) {
        ...
        if (tag.hasKey(CraftMetaFirework.FIREWORKS.NBT)) {
            NBTTagCompound fireworks = tag.getCompoundTag(CraftMetaFirework.FIREWORKS.NBT);

            this.power = 255 & fireworks.getByte(CraftMetaFirework.FLIGHT.NBT);
            if (fireworks.hasKey(CraftMetaFirework.EXPLOSIONS.NBT)) {
                NBTTagList fireworkEffects = fireworks.getTagList(CraftMetaFirework.EXPLOSIONS.NBT, 10);
                List effects = this.effects = new ArrayList(fireworkEffects.tagCount());
    ...
    static FireworkEffect getEffect(NBTTagCompound explosion) {
        FireworkEffect.Builder effect = FireworkEffect.builder().flicker(explosion.getBoolean(CraftMetaFirework.EXPLOSION_FLICKER.NBT)).trail(explosion.getBoolean(CraftMetaFirework.EXPLOSION_TRAIL.NBT)).with(getEffectType(255 & explosion.getByte(CraftMetaFirework.EXPLOSION_TYPE.NBT)));
        int[] aint;
        int i = (aint = explosion.getIntArray(CraftMetaFirework.EXPLOSION_COLORS.NBT)).length;
    ...
    void applyToItem(NBTTagCompound itemTag) {
        ...
        if (!this.isFireworkEmpty()) {
            NBTTagCompound fireworks = itemTag.getCompoundTag(CraftMetaFirework.FIREWORKS.NBT);
    ...
    static void addColors(NBTTagCompound compound, CraftMetaItem.ItemMetaKey key, List colors) {
        if (!colors.isEmpty()) {
    ...
    ImmutableMap.Builder serialize(ImmutableMap.Builder builder) {
        ...
        if (this.hasPower()) {
            builder.put(CraftMetaFirework.FLIGHT.BUKKIT, Integer.valueOf(this.power));
        }
    ...
    static int[] $SWITCH_TABLE$org$bukkit$FireworkEffect$Type() {
        ...
        if (CraftMetaFirework.$SWITCH_TABLE$org$bukkit$FireworkEffect$Type != null) {
            return aint;
        } else {
            int[] aint1 = new int[FireworkEffect.Type.values().length];

            try {
                aint1[FireworkEffect.Type.BALL.ordinal()] = 1;
            } catch (NoSuchFieldError nosuchfielderror) {
                ;
            }

            try {
                aint1[FireworkEffect.Type.BALL_LARGE.ordinal()] = 2;
            } catch (NoSuchFieldError nosuchfielderror1) {
                ;
            }

            try {
                aint1[FireworkEffect.Type.BURST.ordinal()] = 4;
            } catch (NoSuchFieldError nosuchfielderror2) {
                ;
            }

            try {
                aint1[FireworkEffect.Type.CREEPER.ordinal()] = 5;
            } catch (NoSuchFieldError nosuchfielderror3) {
                ;
            }

            try {
                aint1[FireworkEffect.Type.STAR.ordinal()] = 3;
    ...
    static int[] $SWITCH_TABLE$org$bukkit$Material() {
        int[] aint = CraftMetaFirework.$SWITCH_TABLE$org$bukkit$Material;

        if (CraftMetaFirework.$SWITCH_TABLE$org$bukkit$Material != null) {
            return aint;
        } else {
            ...
            CraftMetaFirework.$SWITCH_TABLE$org$bukkit$Material = aint1;
            return aint1;
        }
    }
}
