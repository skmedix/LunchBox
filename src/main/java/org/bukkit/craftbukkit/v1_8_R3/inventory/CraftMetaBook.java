...
public class CraftMetaBook extends CraftMetaItem implements BookMeta {
    ...
    CraftMetaBook(NBTTagCompound tag, boolean handlePages) {
        ...
        if (tag.hasKey(CraftMetaBook.BOOK_TITLE.NBT)) {
            this.title = ValidateUtils.limit(tag.getString(CraftMetaBook.BOOK_TITLE.NBT), 1024);
        }

        if (tag.hasKey(CraftMetaBook.BOOK_AUTHOR.NBT)) {
            this.author = ValidateUtils.limit(tag.getString(CraftMetaBook.BOOK_AUTHOR.NBT), 1024);
        }
        ...
        if (tag.hasKey(CraftMetaBook.BOOK_PAGES.NBT) && handlePages) {
            ...
            for (int i = 0; i < pages.size(); ++i) {
                ...
                this.addPage(new String[] { ValidateUtils.limit(page, 2048)});
            }
        }
        ...
        if (pages != null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (page instanceof String) {
                    this.addPage(new String[] { (String) page});
                }
            }
        }
    ...
    public List getPages() {
        final ImmutableList copy = ImmutableList.copyOf((Collection) this.pages);
    ...
    ImmutableMap.Builder serialize(ImmutableMap.Builder builder) {
        ...
        if (this.hasPages()) {
            ArrayList pagesString = new ArrayList();
            Iterator iterator = this.pages.iterator();
    ...
    static int[] $SWITCH_TABLE$org$bukkit$Material() {
        int[] aint = CraftMetaBook.$SWITCH_TABLE$org$bukkit$Material;

        if (CraftMetaBook.$SWITCH_TABLE$org$bukkit$Material != null) {
            return aint;
        } else {
            ...
            CraftMetaBook.$SWITCH_TABLE$org$bukkit$Material = aint1;
            return aint1;
        }
    }
}
