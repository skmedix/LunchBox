...
class CraftMetaMap extends CraftMetaItem implements MapMeta {
    ...
    ImmutableMap.Builder serialize(ImmutableMap.Builder builder) {
        ...
        if (this.hasScaling()) {
            builder.put(CraftMetaMap.MAP_SCALING.BUKKIT, Boolean.valueOf(this.isScaling()));
        }
    ...
    static int[] $SWITCH_TABLE$org$bukkit$Material() {
        int[] aint = CraftMetaMap.$SWITCH_TABLE$org$bukkit$Material;

        if (CraftMetaMap.$SWITCH_TABLE$org$bukkit$Material != null) {
            return aint;
        } else {
            ...
            CraftMetaMap.$SWITCH_TABLE$org$bukkit$Material = aint1;
            return aint1;
        }
    }
}
