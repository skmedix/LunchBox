...
public class UnsafeList extends AbstractList implements List, RandomAccess, Cloneable, Serializable {
    ...
    public Iterator iterator() {
        ...
        for (int j = 0; j < i; ++j) {
            Iterator newPool = aiterator[j];

            if (!((UnsafeList.Itr) newPool).valid) {
                UnsafeList.Itr iterator = (UnsafeList.Itr) newPool;
        ...
        if (this.iterPool.length < this.maxPool) {
            Iterator[] aiterator1 = new Iterator[this.iterPool.length + 1];

            System.arraycopy(this.iterPool, 0, aiterator1, 0, this.iterPool.length);
            this.iterPool = aiterator1;
