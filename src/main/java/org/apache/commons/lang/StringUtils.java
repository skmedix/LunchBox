                ...
                if (searchLength != replacementLength) {
                    ...
                    for (start = 0; start < searchLength; ++start) {
                        if (!noMoreMatchesForReplIndex[start] && searchList[start] != null && searchList[start].length() != 0 && replacementList[start] != null) {
                            i = text.indexOf(searchList[start]);
                            if (i == -1) {
                                noMoreMatchesForReplIndex[start] = true;
                            } else if (textIndex == -1 || i < textIndex) {
                                textIndex = i;
                                replaceIndex = start;
                            }
                        }
                    ...
                    if (textIndex == -1) {
                        ...
                        StrBuilder strbuilder = new StrBuilder(text.length() + increase);

                        while (textIndex != -1) {
                            for (textLength = start; textLength < textIndex; ++textLength) {
                                strbuilder.append(text.charAt(textLength));
                            }

                            strbuilder.append(replacementList[replaceIndex]);
                            ...
                            for (textLength = 0; textLength < searchLength; ++textLength) {
                                if (!noMoreMatchesForReplIndex[textLength] && searchList[textLength] != null && searchList[textLength].length() != 0 && replacementList[textLength] != null) {
                                    i = text.indexOf(searchList[textLength], start);
                                    if (i == -1) {
                        ...
                        for (int result = start; result < textLength; ++result) {
                            strbuilder.append(text.charAt(result));
                        }
                        ...
                        if (!repeat) {
                            ...
                            return replaceEach(s, searchList, replacementList, repeat, timeToLive - 1);
                        }
                    }
                }
            }
        ...
        if (str == null) {
            ...
            if (repeat != 1 && inputLength != 0) {
                if (inputLength == 1 && repeat <= 8192) {
                    ...
                    default:
                        StrBuilder strbuilder = new StrBuilder(outputLength);
            ...
            if (n == 0) {
                ...
                for (int j = 1; j <= m; ++j) {
                    ...
                    for (i = 1; i <= n; ++i) {
                        ...
                        d[i] = Math.min(Math.min(d[i - 1] + 1, aint[i] + 1), aint[i - 1] + cost);
                    }
