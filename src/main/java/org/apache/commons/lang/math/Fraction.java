...
public final class Fraction extends Number implements Comparable {
    ...
    public static Fraction getFraction(double value) {
        ...
        if (value <= 2.147483647E9D && !Double.isNaN(value)) {
            ...
            do {
                ...
                y2 = x1 - (double) j * y1;
                int k = a1 * numer1 + numer0;

                i = a1 * denom1 + denom0;
                double fraction = (double) k / (double) i;
                ...
                denom1 = i;
                ++i;
            } while (delta1 > delta2 && i <= 10000 && i > 0 && i < 25);
            ...
            if (pos >= 0) {
                ...
                if (pos > 0) {
                    ...
                    if (pos < 0) {
                        ...
                        int denom1 = Integer.parseInt(str.substring(pos + 1));

                        return getFraction(numer, denom, denom1);
                    }
                } else {
                    pos = str.indexOf(47);
                    if (pos < 0) {
    ...
    private Fraction addSub(Fraction fraction, boolean isAdd) {
        if (fraction == null) {
            ...
            if (d1 == 1) {
                int uvp1 = mulAndCheck(this.numerator, fraction.denominator);
                int upv1 = mulAndCheck(fraction.numerator, this.denominator);

                return new Fraction(isAdd ? addAndCheck(uvp1, upv1) : subAndCheck(uvp1, upv1), mulPosAndCheck(this.denominator, fraction.denominator));
            } else {
